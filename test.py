...
def createChannel(ctrlName: list) -> None:
    channelName = "Var"
    for i in ctrlName:
        ...
        if chk:
            ...
            pm.addAttr(i, ln=f'{channelName}', at='float', dv=0)
...
def connectNodes(ctrl: str, objs: list, seRnodes: list, plMnodes: list):
    for i, obj in enumerate(objs):
        seR = seRnodes[i]
        plM = plMnodes[i]
        pm.connectAttr(f"{ctrl}.Var", f"{seR}.valueX", f=True)
        pm.connectAttr(f"{ctrl}.Var", f"{seR}.valueZ", f=True)
...
def main():
    ...
    if num < 5:
        ...
        seR = setRangeNodes(num)
        plM = plusMinusNodes(num)
        connectNodes(ctrl, sel, seR, plM)
        createExpression(sel)
