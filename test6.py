...
__version__ = "Maya2025, Python 3.11.4"
...
def alias(**alias_map):
    ...
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            resolved_kwargs = {alias_map.get(k, k): v for k, v in kwargs.items()}
            return func(*args, **resolved_kwargs)
...
def compare_execution_time(
    func1: Callable, func2: Callable, *args: Any, **kwargs: Any
) -> Tuple[float, float]:
...
def get_bounding_box_position(obj_or_vtx: str) -> tuple:
    ...
    >>> (64.60261, 67.08806, -62.83971)
    """
    bounding_box = cmds.xform(obj_or_vtx, q=True, bb=True, ws=True)
...
def get_distance(point1: Union[tuple, list], point2: Union[tuple, list]) -> float:
...
def create_pole_vector_joints(*joints) -> list:
    ...
    pm.aimConstraint(end_jnt, result[0], o=(0, 0, 90), wut="object", wuo=middle_jnt)
...
def parent_in_sequence(*objs) -> list:
    ...
    result = []
    for obj, child in zip(sel, sel[1:]):
        result.append(obj)
        try:
...
def group_with_pivot(*args, **kwargs) -> list:
    ...
    for i in sel:
        ...
        except (pm.MayaCommandError, RuntimeError) as e:
            pm.warning(e)
        result += temp
...
@use_selection
def create_curve(
    *args, degree: int = 1, closed_curve: bool = False, curve_name: str = ""
) -> str:
...
def create_aimed_curve(
    start_obj_or_vtx: str, end_obj_or_vtx: str, world_up_object: str = ""
) -> str:
...
@use_selection
def create_animation_curves(*obj_or_vtx, start_frame: int, end_frame: int) -> list:
...
def set_key_on_range(
    object: str, start_frame: int, end_frame: int, rotation: bool = False
) -> None:
...
def create_follicle(mesh: str, UVCoordinates: tuple, uv_set="map1") -> str:
    ...
    pm.connectAttr(
        f"{follicle_shape}.outTranslate", f"{follicle_node}.translate", f=True
    )
    pm.connectAttr(f"{follicle_shape}.outRotate", f"{follicle_node}.rotate", f=True)
    pm.connectAttr(f"{deformed_shape}.outMesh", f"{follicle_shape}.inputMesh", f=True)
    pm.connectAttr(
        f"{mesh}.worldMatrix[0]", f"{follicle_shape}.inputWorldMatrix", f=True
    )
    u, v = UVCoordinates
...
def create_blendColor_node(
    ...
    for attr in blend_attrs:
        ...
        pm.connectAttr(f"{ctrl}.{ctrl_attr}", f"{blend_node}.blender", force=True)
        pm.connectAttr(f"{fk_joint}.{attr}", f"{blend_node}.color1", force=True)
        pm.connectAttr(f"{ik_joint}.{attr}", f"{blend_node}.color2", force=True)
...
@use_selection
def extract_numbers(
    text: str, last_position_of_number: bool = True
...
def get_symmetric_vertex(
    *vertex_strings: str, axis: str = "x", tolerance: float = 0.005
) -> dict:
...
def get_vertex_weights(*vertices):
    ...
    for vtx in vertices:
        ...
        weights_map = {inf.name(): w for inf, w in zip(influences, weights) if w > 0.0}
...
def colorize(*args, color_idx=None, color_rgb=()) -> None:
    ...
    This function iterates through the provided Maya nodes, finds their
    visible shape nodes, and applies a color override. You can specify
    the color using either a color index or an RGB tuple.
    If `color_idx` is provided, it takes precedence over `color_rgb`.

    Args:
        *args:
            Variable-length argument list of Maya node names or PyNode objects.
            Each argument should be a string representing a node name or
            a PyNode object.
            - "gray": (0.534, 0.534, 0.534)
            - "black": (0, 0, 0)
            - "dark_gray": (0.332, 0.332, 0.332)
            ...
            - "white": (1, 1, 1)
            - "yellow": (1, 1, 0)
            - "light_cyan": (0.673, 1, 1)
            - "pale_green": (0.616, 1, 0.648)
            - "light_pink": (1, 0.78, 0.761)
            - "peach": (1, 0.76, 0.545)
            - "chartreuse": (0.840, 1, 0)
            - "forest_green": (0.443, 0.645, 0.426)
            - "tan": (0.631, 0.497, 0.291)
            - "khaki": (0.675, 0.693, 0.324)
            - "sage_green": (0.548, 0.683, 0.324)
        ...
        color_idx (int, optional):
            The color index to apply. This corresponds to Maya's internal
            color palette. Defaults to None.
        color_rgb (tuple, optional):
            A tuple of three floats (R, G, B) representing the RGB color
            values (0.0 to 1.0). Defaults to an empty tuple.

    Raises:
        RuntimeError:
            If a provided argument cannot be converted to a PyNode or
            if the node does not have override attributes.
    ...
    for arg in args:
        ...
        for shp in shapes:
            if shp.hasAttr("overrideEnabled"):
                shp.overrideEnabled.set(1)
                if color_idx:
                    shp.overrideRGBColors.set(0)
                    shp.overrideColor.set(color_idx)
                elif color_rgb:
...
class ColorPickerUI:
    ...
    def show(self):
        ...
        pm.button(label="Apply", height=btn_h, c=self.apply_color)
