...
class Joint:
    ...
    arm_L = [f'jnt_{i}_L' for i in arm]
    arm_R = [f'jnt_{i}_R' for i in arm]
    leg = ['hip', 'knee', 'ankle', 'ball', 'toe']
    leg_L = [f'jnt_{i}_L' for i in leg]
    leg_R = [f'jnt_{i}_R' for i in leg]
...
# def createJoint():
#     jnt = []
#     for jnt, pos in Joint.all.items():
#         pm.select(cl=True)
